...
class SigLipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, get_attn_weights=None) -> torch.Tensor:
        patch_embeds = self.patch_embedding(pixel_values)  # shape = [*, width, grid, grid]
        embeddings = patch_embeds.flatten(2).transpose(1, 2)

        if hasattr(self, "position_embedding_norm") and get_attn_weights:
            embeddings = embeddings + self.position_embedding_norm(self.position_ids)
        else:
...
def get2pool2d(image_feature, stride=2):
...
class SigLipVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        output_hidden_states = output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        hidden_states = self.embeddings(pixel_values, get_attn_weights)
...
class SigLipMultiheadAttentionPoolingHead(nn.Module):
    ...
    def forward(self, hidden_state, attn_weights_bool=False):
...
class SigLipVisionTower(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            if wrapper in ["holitom", 'prunevid', 'flashvid', 'fastvid']:
                image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, output_attentions=True)
                attn_weights  = image_forward_outs.attentions[-1]
                hidden_states = image_forward_outs.hidden_states[-1]
...
class SigLipVisionAbstract(nn.Module):
    ...
    def forward(self, images):
        ...
        pooled_output, attn_weights = self.vision_abstract.vision_model.head(last_hidden_state, attn_weights_bool=True)
