...
class LlavaMetaForCausalLM_holitom(ABC):
    ...
    def cluster_dpc_knn(self, x, cluster_num, k=7, static_sink_score=None):
        with torch.no_grad():
            ...
            score = dist * density
            if static_sink_score is not None:
                score_ = score - static_sink_score[:1,:] * 0.3
                _, index_center = score_.topk(cluster_num, dim=-1)
            else:
    ...
    def merge_tokens_by_attention_density(self, feat, attn, pos, retain_ratio, D, Beta, K, args, sink_score):
        batch_size, seq_len, embed_dim = feat.shape
        dominant_num = round(math.ceil(seq_len * retain_ratio) * (1-D))
        contextual_num = dominant_num - dominant_num
        
        ## Dominant Visual Tokens
        if dominant_num > 0:
            attn_ = attn  - args.mu_s * sink_score # mu: 0.1, lambda: 0.05
            all_indices = attn_.topk(dominant_num, dim=1).indices
    ...
    def merge_tokens_by_density(self, feat, pos, retain_ratio, Beta, K, args=None, static_sink_score=None):
        batch_size, seq_len, embed_dim = feat.shape
        cluster_num = round(seq_len * retain_ratio)
        if cluster_num > 0:
            target_indices, dist_matrix = self.cluster_dpc_knn(feat, cluster_num, k=min(K,cluster_num), static_sink_score=static_sink_score)
            target_indices = torch.sort(target_indices, dim=-1)[0]
    ...
    def holitom(self, static_feat, dynamic_feat, dynamic_attn, static_pos, dynamic_pos, window_size, retain_ratio, D, Beta, K, images_dtype, mm_newline_position, args, dynamic_sink_score, static_sink_score):
        ...
        if window_size==1:
            dynamic_feat, dynamic_pos = self.merge_tokens_by_attention_density(dynamic_feat, dynamic_attn, dynamic_pos, retain_ratio, D, Beta, K, args, dynamic_sink_score)
            ...
            dynamic_feat, dynamic_pos = self.merge_tokens_by_attention_density(dynamic_feat, dynamic_attn, dynamic_pos, retain_ratio, D, Beta, K, args, dynamic_sink_score) # Non-Redundant
            static_feat, static_pos = self.merge_tokens_by_density(static_feat.unsqueeze(0), static_pos, retain_ratio, Beta, K, args, static_sink_score) ## Redundant
            if mm_newline_position != "grid":
    ...
    def holitom_no_spatial(self, static_feat, dynamic_feat, dynamic_attn, static_pos, dynamic_pos, window_size, retain_ratio, D, Beta, K, images_dtype, mm_newline_position):
        newline_token = self.model.image_newline[None].to(static_feat.device) if mm_newline_position == "grid" else None
        grid_size = int(math.sqrt(dynamic_feat.shape[1]+static_feat.shape[0]))
    ...
    def get_static_dynamic_features(self, image_feat, attn_weights, selected_frames, feature_sim, tau, retain_ratio, args=None, sink_score = None): # Temporal Merging
        ...
        dynamic_sink_score_list = []
        static_sink_score_list = []
        for start,end in selected_frames:
            ...
            if start == end:
                ...
                dynamic_sink_score_list.append(sink_score[start:end+1])
                static_sink_score_list.append(torch.empty((0, 1), device=image_feat.device))
            else:
                windows_size = end-start+1
                
                cos = feature_sim[start:end, :]
                sink = sink_score[start:end]
                score = cos + args.mu_t * sink
                mask = torch.all(score>0.8, dim=0)
                dynamic_sink_score_list.append(sink_score[start:end+1, ~mask])

                static_sink_score_list.append(sink_score[start:end+1, mask])
                ...
                static_feat_list.append(static_feat.mean(dim=0)) ## Merging with temporal axis
        ...
        return static_feat_list, dynamic_feat_list, _, dynamic_attn_list, static_pos_list, dynamic_pos_list, dynamic_sink_score_list, static_sink_score_list
        ...
        if type(images) is list or images.ndim == 5:
            ...
            for idx, image_feat in enumerate(encoded_image_features):
                if idx in video_idx_in_batch:
                    ...
                    sink_score = minmax_norm(attn_weights.mean(0).repeat(pooled_image_feat_normed.shape[0], 1)**(args.gamma))
                    ...
                    static_feat, dynamic_feat, _, dynamic_attn, static_pos, dynamic_pos, dynamic_sink_score_list, static_sink_score_list = self.get_static_dynamic_features(
                        pooled_image_feat, attn_weights, selected_frames,
                        feature_sim, tau, retain_ratio, args, sink_score
                    )

                    segment_features = []
                    all_attention_sink_pos = []
                    all_norm_sink = []
                    for idx, (start,end) in enumerate(selected_frames):
                        window_size = end - start + 1

                        image_feat = self.holitom(
                                static_feat[idx], dynamic_feat[idx], dynamic_attn[idx], static_pos[idx], dynamic_pos[idx],
                                window_size, retain_ratio, D, Beta, K, images_dtype, mm_newline_position, args, dynamic_sink_score_list[idx], static_sink_score_list[idx]
                            ) # image_feat: [static, dynamic]
                            
                        segment_features.append(image_feat)
