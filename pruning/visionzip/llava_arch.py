...
def standarization(x: torch.Tensor, eps: float = 1e-12):
    avg = x.mean(dim=1)
    std = x.std(dim=1)
    normalization = (x-avg[...,None]) / std[...,None]
...
class LlavaMetaForCausalLM_visionzip(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None, args=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            encoded_image_features, attn_weights, metric, images_dtype = self.encode_images_multi(concat_images, args)
            attn_weights_full = attn_weights
            ...
            for idx, image_feat in enumerate(encoded_image_features):
                if idx in video_idx_in_batch:
                    ...
                    def visionzip(hidden_states, attention, metric, dominant_num, contextual_num, images_dtype, mm_newline_position, attn_weights_full):
                        ...
                        sink_score = minmax_norm(attn_weights.mean(0).repeat(pooled_image_feat.shape[0], 1)**(args.gamma))
                        attn_ = attention - args.mu_s * sink_score # mu: 0.1, lambda: 0.05
                        topk_indices = attn_.topk(dominant_num, dim=1).indices  # (batch_size, dominant_num)
                        ...
                        sink_indices = sink_score[0].topk(20).indices
                        target_indices = torch.arange(0, seq_len-dominant_num, step, device=metric_normalized.device)[:contextual_num]
                        new_target_indices = []
                        for t in target_indices:
                            if t not in sink_indices:
                                new_target_indices.append(t)
                            else:
                                is_sink = True
                                while is_sink and t < seq_len-dominant_num:
                                    t += 1
                                    if t not in sink_indices:
                                        new_target_indices.append(t)
                                        is_sink = False
                                if is_sink:
                                    new_target_indices.append(t)
                        target_indices = torch.hstack(new_target_indices)
                        ...
                        for j, i in enumerate(~mask):
                            select_visual_idx.append(i.nonzero().view(-1).tolist()+target_indices.cpu().tolist())
                        select_visual_idx = np.stack(select_visual_idx)
                        sampled_token_idx_list.append(select_visual_idx)
                        ...
                        return image_features
                    img_feat = visionzip(pooled_image_feat, attn_weights, metric, dominant_num, contextual_num, images_dtype, mm_newline_position, attn_weights_full)
