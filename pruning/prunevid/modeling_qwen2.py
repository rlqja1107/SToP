...
class Qwen2Model_prunevid(Qwen2PreTrainedModel):
    """
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        all_attention_sink_pos = self.all_attention_sink_pos
        for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            if output_hidden_states:
                all_hidden_states += (hidden_states.cpu(),)
                # all_hidden_states += [np.round(hidden_states.cpu().numpy(), 4)]
            ...
            #    attn_output_mean = layer_outputs[1][0].mean(0)
            # # all_self_attns += (np.round(attn_output_mean.cpu().numpy(), 4),)
            #    all_self_attns += (attn_output_mean.cpu(),)
            
            
        all_token_idx = (14, all_attention_sink_pos.cpu(), hidden_states.shape[1] - 14 - len(all_attention_sink_pos))
        hidden_states = self.norm(hidden_states)
        if output_hidden_states:
            # all_hidden_states += [np.round(hidden_states.cpu().numpy(), 4)]
            all_hidden_states += (hidden_states.cpu(),)

        # with open("Analyze/visual_attention_sink/holitom_mcqa_mvbench_spatial_temporal/attn_token_idx_hidden_states.pkl", 'wb') as f:
        #     pickle.dump(all_self_attns, f)
        #     pickle.dump(all_token_idx, f)
