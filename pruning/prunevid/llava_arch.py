...
class LlavaMetaForCausalLM_prunevid(ABC):
    ...
    def cluster_dpc_knn(self, x, cluster_num, k=5, token_mask=None, static_indices=None, args=None):
        with torch.no_grad():
            B, N, C = x.shape
        ...
        if static_indices is not None:
            return idx_cluster, cluster_num, static_indices[index_down]
        else:
    ...
    def spatial_merge_tokens(self, feature, num_cluster, k, static_indices=None, args=None):
        cluster_idx, _, cluster_center = self.cluster_dpc_knn(feature, cluster_num=num_cluster, k=k, static_indices=static_indices, args=args)
        feature = self.compute_cluster_vectors(feature, cluster_idx, num_cluster=num_cluster)
        return feature, cluster_center


    def merge_frames_dynamic(self, frames, threshold=0.8, k=7, args=None, min_frame=None, attn_weights=None):
        B, L, C = frames.shape
        # assert L == args.n_frame * 14 * 14
        frames = frames.view(B, min(args.n_frame, min_frame), 14*14, C) # B T L C
        idx_clusters, _, _ = self.cluster_dpc_knn(frames.mean(dim=2), cluster_num=int(args.n_frame*0.25), k=k, args=args)
        idx_clusters = self.refine_clusters(idx_clusters)
        window_list = self.segment_lengths(idx_clusters)
        ...
        start_idx = 0
        dist_matrix = torch.cdist(frames.float(), frames.float()) / (frames.shape[-1]**0.5)
        density = (-(dist_matrix**2).mean(dim=-1)).exp()[0]
        ...
        for window_size in window_list[0]:
            ...
            retain_ratio = min((args.retention_ratio * L*window_size) / (static_feat.shape[1]+(L-static_feat.shape[1])*window_size.item())+0.005, 1)
            static_indices = mask[0].nonzero()[:,-1]
            static_cluster_center = mask[0].nonzero()[:,0]
            if static_feat.shape[1] > 14:
                static_feat, static_cluster_center = self.spatial_merge_tokens(static_feat, num_cluster=int(static_feat.shape[1]*retain_ratio), k=7, static_indices=static_indices, args=args)
                static_cluster_center = static_cluster_center[0]
            static_features.append(static_feat)
            static_sizes.append(static_feat.shape[1])
            cluster_center_list.append(static_cluster_center)
            
            dynamic_mask = ~mask_expand
            dynamic_feat = torch.masked_select(current_frames, dynamic_mask).view(B, window_size, -1, C)
            dynamic_window_list = []
            indices = dynamic_mask[0][0].all(-1).nonzero()[:,-1]
            for i in range(window_size):
                dynamic_feat_window = dynamic_feat[:,i,:,:]
                dynamic_cluster_center = indices
                if dynamic_feat_window.shape[1] > 14:
                    dynamic_feat_window, dynamic_cluster_center = self.spatial_merge_tokens(dynamic_feat_window, num_cluster=int(dynamic_feat_window.shape[1]*retain_ratio), k=7, static_indices = indices, args=args)
                    dynamic_cluster_center = dynamic_cluster_center[0]
                dynamic_window_list.append(dynamic_feat_window)
                cluster_center_list.append(dynamic_cluster_center)
        ...
        return final_features, static_sizes, dynamic_sizes, window_sizes, cluster_center_list
        ...
        if type(images) is list or images.ndim == 5:
            ...
            for idx, image_feat in enumerate(encoded_image_features):
                if idx in video_idx_in_batch:
                    ...
                    image_feat, static_sizes, dynamic_sizes, window_sizes, cluster_center_list = self.merge_frames_dynamic(pooled_image_feat.reshape(1, -1, pooled_image_feat.shape[2]), threshold=threshold, k=7, args=args, min_frame = pooled_image_feat.shape[0], attn_weights=attn_weights)
